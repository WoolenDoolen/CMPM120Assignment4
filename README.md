...
## 6. How to Run the Game (Local Setup)

1. Open the project folder in VS Code (or another editor).
2. Run a simple local server in the folder (examples):
   - Using **five-server**:
     - `npx five-server .` (or your existing five-server command)
   - Or using **http-server**:
     - `npx http-server .`
3. Open the provided local URL (e.g., `http://localhost:5500` or similar).
4. Make sure the folder structure matches the paths used in code:
   - `index.html`
   - `index.js` (or `main.js`, whichever you’re using in `<script>` tag)
   - `tilemap/Project 4.tmj`
   - `tilemap/tilemap_packed.png`
   - `assets/Player sprites/player/...`
   - `assets/Enemy sprites/...`

Once the page loads, you’ll see the **title screen**. Press **ENTER** to start playing.
